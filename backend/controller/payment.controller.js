    ...
    try {
        ...
        if(!Array.isArray(products) || products.length === 0){
            return res.status(400).json({
                error: 'Invalid or empty product array'
            })
        }
        let totalAmount = 0
        const lineItems = products.map(product => {
            const amount = Math.round(product.price * 100) //Stripe send the amount in cents(cents => $10 * 100 = 1000)
        ...
        //if user total order amount is greater than 20000, create a new coupon
